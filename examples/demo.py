...
from agentic.llm_deepseek import DeepSeekLLM
...
def main() -> None:
    ...
    # Инициализируем LLM-клиент. Ключ берется из .env или окружения.
    llm = DeepSeekLLM()

    logger.info("== Цепочка промптов ==")
    # Черновик -> улучшение.
    ...
    logger.info("== Маршрутизация ==")
    # Роутер выбирает ветку, затем вызывается соответствующий обработчик.
    route, response = route_query(llm, "Посчитай 2 + 2 * 5")
    logger.info("Маршрут: %s", route)
    logger.info("Ответ: %s", response)

    logger.info("== Использование инструментов ==")
    # Агент выбирает инструмент, вызывает его и формирует ответ.
    answer = tool_use_agent(llm, "Что такое Python?")
    logger.info("Ответ: %s", answer)

    logger.info("== Планирование ==")
    # Планируем шаги и исполняем их (заглушки).
